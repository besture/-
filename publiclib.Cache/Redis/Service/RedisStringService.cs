...
namespace publiclib.Cache.Redis.Service
{
    ...
    public class RedisStringService : RedisHelper
    {
        ...
        public RedisStringService(int? dbNum, string redisHostAddress) : base(dbNum, redisHostAddress) { }

        public RedisStringService(string customkey, int? dbNum = null, string readWriteHosts = null)
              : this(dbNum, readWriteHosts)
        {
        ...
        public TimeSpan? KeyTimeToLive(string key)
        {
            ...
            return base.redis.KeyTimeToLive(key);
        }
        ...
        public long StringAppend<T>(string key, T value)
        {
            key = AddSysCustomKey(key);
            string jsonValue = ConvertJson(value);
            return base.redis.StringAppend(key, jsonValue);
        }
        ...
        /// <param name="key">要获取值的Key集合</param>
        /// <returns></returns>
        public T ParkStringGet<T>(string key,string CustomKey)
        {
            key = CustomKey + ":" + key; ;
            var values = base.redis.StringGet(key);
            return ConvertObj<T>(values);
